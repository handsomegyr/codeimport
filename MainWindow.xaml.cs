...
namespace ExcelApplication1
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            bool isOk = false;
            if (this.objSettings.Quantity > 0)
            {
                isOk = Worker.randomCreate( this.objSettings, backgroundWorker);
            }
            else
            {
                isOk = Worker.merge(selectedPath, excelFileList, this.objSettings, backgroundWorker);
            }            
        ...
        {
            var objSettingsWindow = new RandomSettingsWindow();
            objSettingsWindow.setSettings(this.objSettings);
            if (objSettingsWindow.ShowDialog().Value)
            {
                this.objSettings = objSettingsWindow.getSettings();
            ...
            button1.IsEnabled = false;
            cmdCancel.IsEnabled = true;
            this.progressBar1.Maximum = this.objSettings.Quantity;
            // Start the process on another thread.
            backgroundWorker.RunWorkerAsync();
        }
    }
